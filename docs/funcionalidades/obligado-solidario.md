...
## Arrendador anterior
Datos: nombre, celular, teléfono fijo, correo, **dirección del inmueble que rentaba** (`clients.previous_landlord_address`) y tiempo de arrendamiento. El cliente los captura en su Portal (Ingresos) y el asesor los ve, llama/WhatsApp y **captura o corrige por teléfono** en Renta → Investigación (`rentals/_previous_landlord.blade.php`, `RentalProcessController::savePreviousLandlord`, ruta `rentals.previous-landlord.save`; inquilino y obligado). Sin `<form>` anidado (`hdvLandlordSave`).
