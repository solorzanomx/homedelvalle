...
// ── Test Mifiel (temporal — quitar en producción) ────────────────────────────
Route::middleware(['auth', 'admin'])->get('/test-mifiel', function () {
    // Generar PDF dummy con DomPDF
    $html = '<html><body style="font-family:sans-serif;padding:40px;">
        <h1>Contrato de Prueba</h1>
        <p>Este es un documento de prueba para Mifiel — Sandbox.</p>
    ...
    $pdfPath = storage_path('app/private/test-mifiel-dummy.pdf');
    file_put_contents($pdfPath, $dompdf->output());

    // Enviar a Mifiel sandbox
    try {
        $mifiel = app(\App\Services\MifielService::class);

        $result = $mifiel->sendDocument(
            pdfPath:     $pdfPath,
            signers:     [
                [
            ...
            callbackUrl: url('/api/webhooks/mifiel'),
        );

        // Guardar en BD de seguimiento
        \App\Models\MifielDocumento::create([
            'document_id'     => $result['document_id'],
            'tipo'            => 'test',
            'status'          => 'pending',
            'mifiel_response' => $result,
        ]);

        return response()->json([
            'ok'      => true,
            'message' => 'Documento enviado a Mifiel sandbox',
            'result'  => $result,
        ...
        return response()->json([
            'ok'    => false,
            'error' => $e->getMessage(),
            'hint'  => 'Verifica MIFIEL_APP_ID y MIFIEL_APP_SECRET en .env y que MIFIEL_SANDBOX=true',
