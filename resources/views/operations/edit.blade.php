...
@section('title', 'Editar Operacion')
...
<div class="page-header">
    <div>
        ...
        <p class="text-muted">{{ $operation->type_label }} #{{ $operation->id }}</p>
    </div>
...
<div class="card" style="max-width:800px;">
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('operations.update', $operation) }}" id="operationForm">
            @csrf @method('PUT')

            {{-- Tipo de Operacion --}}
            <div class="section-title" style="margin-top:0;">Tipo de Operacion</div>
            <div class="type-cards">
                <label class="type-card {{ old('type', $operation->type) === 'venta' ? 'selected' : '' }}" id="cardVenta">
                    <input type="radio" name="type" value="venta" {{ old('type', $operation->type) === 'venta' ? 'checked' : '' }} required onchange="onTypeChange(this.value)">
                    <div class="type-icon">&#9830;</div>
                    <div class="type-label">Venta</div>
                    <div class="type-desc">Compraventa de propiedad</div>
                </label>
                <label class="type-card {{ old('type', $operation->type) === 'renta' ? 'selected' : '' }}" id="cardRenta">
                    <input type="radio" name="type" value="renta" {{ old('type', $operation->type) === 'renta' ? 'checked' : '' }} required onchange="onTypeChange(this.value)">
                    <div class="type-icon">&#127968;</div>
                    <div class="type-label">Renta</div>
                    <div class="type-desc">Arrendamiento de propiedad</div>
                </label>
                <label class="type-card {{ old('type', $operation->type) === 'captacion' ? 'selected' : '' }}" id="cardCaptacion">
                    <input type="radio" name="type" value="captacion" {{ old('type', $operation->type) === 'captacion' ? 'checked' : '' }} required onchange="onTypeChange(this.value)">
                    <div class="type-icon">&#128204;</div>
            ...
            <div class="conditional-field {{ old('type', $operation->type) === 'captacion' ? 'visible' : '' }}" id="fieldTargetType">
                <div class="form-group" style="margin-top:0.75rem;">
                    <label class="form-label">Destino de la captacion <span class="required">*</span></label>
                    <select name="target_type" class="form-select">
                        <option value="">-- Seleccionar --</option>
                        <option value="venta" {{ old('target_type', $operation->target_type) === 'venta' ? 'selected' : '' }}>Venta</option>
                        <option value="renta" {{ old('target_type', $operation->target_type) === 'renta' ? 'selected' : '' }}>Renta</option>
                    </select>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <select name="property_id" class="form-select">
                        ...
                        @foreach($properties as $property)
                            <option value="{{ $property->id }}" {{ old('property_id', $operation->property_id) == $property->id ? 'selected' : '' }}>{{ $property->title }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="client_id" class="form-select" required>
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ old('client_id', $operation->client_id) == $client->id ? 'selected' : '' }}>{{ $client->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="secondary_client_id" class="form-select">
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ old('secondary_client_id', $operation->secondary_client_id) == $client->id ? 'selected' : '' }}>{{ $client->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="broker_id" class="form-select">
                        ...
                        @foreach($brokers as $broker)
                            <option value="{{ $broker->id }}" {{ old('broker_id', $operation->broker_id) == $broker->id ? 'selected' : '' }}>{{ $broker->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-grid">
                ...
                <div class="form-group conditional-field" id="fieldAmount">
                    ...
                    <input type="number" name="amount" class="form-input" value="{{ old('amount', $operation->amount) }}" min="0" step="0.01" placeholder="0.00">
                ...
                <div class="form-group conditional-field" id="fieldMonthlyRent">
                    ...
                    <input type="number" name="monthly_rent" class="form-input" value="{{ old('monthly_rent', $operation->monthly_rent) }}" min="0" step="0.01" placeholder="0.00">
                ...
                <div class="form-group conditional-field" id="fieldDeposit">
                    ...
                    <input type="number" name="deposit_amount" class="form-input" value="{{ old('deposit_amount', $operation->deposit_amount) }}" min="0" step="0.01" placeholder="0.00">
                ...
                <div class="form-group conditional-field" id="fieldGuarantee">
                    ...
                    <select name="guarantee_type" class="form-select">
                        ...
                        @foreach(\App\Models\Operation::GUARANTEE_TYPES as $val => $label)
                            <option value="{{ $val }}" {{ old('guarantee_type', $operation->guarantee_type) === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="commission_amount" class="form-input" value="{{ old('commission_amount', $operation->commission_amount) }}" min="0" step="0.01" placeholder="0.00">
                    @error('commission_amount') <p class="form-hint" style="color:var(--danger)">{{ $message }}</p> @enderror
                </div>
                <div class="form-group">
                    <label class="form-label">Comision (%)</label>
                    <input type="number" name="commission_percentage" class="form-input" value="{{ old('commission_percentage', $operation->commission_percentage) }}" min="0" max="100" step="0.01" placeholder="0.00">
                    @error('commission_percentage') <p class="form-hint" style="color:var(--danger)">{{ $message }}</p> @enderror
                </div>
                <div class="form-group">
                    <label class="form-label">Moneda</label>
                    <select name="currency" class="form-select">
                        <option value="MXN" {{ old('currency', $operation->currency ?? 'MXN') === 'MXN' ? 'selected' : '' }}>MXN</option>
                        <option value="USD" {{ old('currency', $operation->currency) === 'USD' ? 'selected' : '' }}>USD</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="expected_close_date" class="form-input" value="{{ old('expected_close_date', $operation->expected_close_date?->format('Y-m-d')) }}">
            ...
            <div class="form-group">
                <textarea name="notes" class="form-textarea" rows="4" placeholder="Notas adicionales sobre la operacion...">{{ old('notes', $operation->notes) }}</textarea>
                @error('notes') <p class="form-hint" style="color:var(--danger)">{{ $message }}</p> @enderror
            </div>

            <div class="meta-info">
                Creado: {{ $operation->created_at->format('d/m/Y H:i') }} &middot;
                Actualizado: {{ $operation->updated_at->format('d/m/Y H:i') }}
            </div>
        ...
        <div class="delete-section">
            <p><strong>Zona de peligro:</strong> Esta accion eliminara la operacion y todos sus datos asociados de forma permanente.</p>
            <form method="POST" action="{{ route('operations.destroy', $operation) }}" onsubmit="return confirm('Seguro que deseas eliminar esta operacion? Esta accion no se puede deshacer.')">
                @csrf @method('DELETE')
                <button type="submit" class="btn btn-danger">Eliminar Operacion</button>
            </form>
        </div>
    </div>
</div>
