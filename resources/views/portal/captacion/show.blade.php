...
@section('styles')
    ...
    .etapa-step.done .etapa-num::after { content:' \2713'; }
    .doc-item { display:flex; align-items:center; gap:.75rem; padding:.75rem 0; border-bottom:1px solid var(--border); font-size:.85rem; }
                    ...
                    <div style="flex:1;">
                        ...
                        @if($latestDoc)
                            <div style="font-size:.75rem; color:var(--text-muted); margin-top:.1rem;">
                                {{ $latestDoc->file_name }} &middot;
                                @if($docStatus === 'aprobado')
                                    <span style="color:var(--success);">Aprobado</span>
                                @elseif($docStatus === 'rechazado')
                                    <span style="color:var(--danger);">Rechazado{{ $latestDoc->rejection_reason ? ': ' . $latestDoc->rejection_reason : '' }}</span>
                                @else
...
{{-- ── Etapa 2: Valuación ── --}}
@php $dimmed2 = $etapa < 2 ? 'opacity:.55;pointer-events:none;' : ''; @endphp
<div class="card" style="margin-bottom:1.25rem; {{ $dimmed2 }}">
    <div class="card-header">
        <h3>
            @if($etapa > 2)
                <span style="color:var(--success);">&#10003; </span>
            @endif
            Etapa 2 &mdash; Valuación del Inmueble
        </h3>
        @if($etapa === 2)
            <span class="badge badge-blue">En curso</span>
        @elseif($etapa > 2)
...
{{-- ── Etapa 3: Precio ── --}}
@php $dimmed3 = $etapa < 3 ? 'opacity:.55;pointer-events:none;' : ''; @endphp
<div class="card" style="margin-bottom:1.25rem; {{ $dimmed3 }}">
    <div class="card-header">
        <h3>
            @if($etapa > 3)
                <span style="color:var(--success);">&#10003; </span>
            @endif
            Etapa 3 &mdash; Acuerdo de Precio
        </h3>
        @if($etapa === 3)
            <span class="badge badge-blue">En curso</span>
        @elseif($etapa > 3)
...
@php $dimmed4 = $etapa < 4 ? 'opacity:.55;pointer-events:none;' : ''; @endphp
<div class="card" style="{{ $dimmed4 }}">
    <div class="card-header">
        <h3>
            @if($captacion->isEtapa4Complete())
                <span style="color:var(--success);">&#10003; </span>
            @endif
            Etapa 4 &mdash; Contrato de Exclusiva
        </h3>
        @if($etapa < 4)
            <span class="badge badge-yellow">Pendiente</span>
