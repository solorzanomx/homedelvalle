...
@elseif($tenantMode)
    ...
    <p style="font-size:.85rem;color:#64748b;margin:0 0 1rem;line-height:1.5;">Completa cada sección con calma: se guarda al enviarla. {{ $secDone }} de {{ $secTotal }} secciones listas.</p>
@else
<div class="exp-hero">
...
@if($isArrendatario && $rentalAsInquilino && (! ($wizard ?? null) || request()->boolean('apartado')))
    ...
    <div style="display:flex;align-items:center;gap:.6rem;flex-wrap:wrap;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:.6rem .9rem;margin-bottom:1rem;font-size:.82rem;color:#166534;">
        <span>✅ <strong>Apartado confirmado</strong> el {{ $rentalAsInquilino->apartado_paid_at->format('d/m/Y') }} (${{ number_format($rentalAsInquilino->apartado_amount, 2) }} MXN)</span>
        @if($reciboApartado)<a href="{{ route('portal.documents.download', $reciboApartado->id) }}" style="margin-left:auto;font-weight:700;color:#166534;">📄 Recibo</a>@endif
    </div>
    @else
    <div class="apartado-card pending" id="apartado">
