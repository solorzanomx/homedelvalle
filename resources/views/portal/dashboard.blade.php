...
<div class="portal-grid">
    ...
    <div>
        {{-- Mi Inmueble card --}}
        @if($isVenta)
        <a href="{{ route('portal.mi-inmueble') }}" style="display:block;text-decoration:none;margin-bottom:1.25rem;">
            <div class="section-card" style="border-left:3px solid #0E304B;transition:box-shadow .15s;"
                 onmouseover="this.style.boxShadow='0 4px 12px rgba(0,0,0,.08)'"
                 onmouseout="this.style.boxShadow='none'">
                <div class="section-body" style="padding:.9rem 1.25rem;display:flex;align-items:center;justify-content:space-between;">
                    <div>
                        <div style="font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#0E304B;margin-bottom:.25rem;">Mi Inmueble</div>
                        <div style="font-weight:700;font-size:.9rem;color:#0E304B;">Actividad y métricas</div>
                        <div style="font-size:.78rem;color:var(--text-muted);margin-top:2px;">Visitas, reacciones y pulso de interés</div>
                    </div>
    ...
    <div>
        ...
        {{-- Advisor --}}
        @php
            $advisorName = $captacion?->createdBy?->name ?? $client->assignedUser?->name ?? 'Tu Asesor';
            $advisorInitial = strtoupper(substr($advisorName, 0, 1));
            $advisorPhone = $captacion?->createdBy?->phone ?? $client->assignedUser?->phone ?? null;
            $waLink = $advisorPhone
                ? 'https://wa.me/' . preg_replace('/[^0-9]/', '', $advisorPhone) . '?text=Hola,' . urlencode(' tengo una pregunta sobre mi proceso')
                : 'https://wa.me/525513450978?text=Hola%2C+tengo+una+pregunta+sobre+mi+proceso';
        @endphp
        <div class="advisor-card">
            <div class="advisor-avatar">{{ $advisorInitial }}</div>
