    ...
    <div class="min-h-screen flex items-center justify-center px-4 py-8">
        <div class="w-full max-w-2xl">
            <div class="mb-8">
                ...
                <h1 class="text-4xl font-bold text-slate-900 dark:text-slate-100 mt-4">🤝 Editar Broker</h1>
                <p class="text-slate-600 dark:text-slate-400 mt-2">{{ $broker->name }}</p>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-8">
                ...
                <form action="{{ route('brokers.update', $broker->id) }}" method="POST" class="space-y-6">
                    @csrf @method('PUT')
                    ...
                    <div class="border-b border-slate-200 dark:border-slate-700 pb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="name" value="{{ $broker->name }}" required class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none dark:bg-slate-700 dark:text-slate-200">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Email *</label>
                                <input type="email" name="email" value="{{ $broker->email }}" required class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none dark:bg-slate-700 dark:text-slate-200">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            <div>
                                ...
                                <input type="tel" name="phone" value="{{ $broker->phone }}" class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none dark:bg-slate-700 dark:text-slate-200">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Empresa</label>
                                <input type="text" name="company_name" value="{{ $broker->company_name }}" class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none dark:bg-slate-700 dark:text-slate-200">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border-b border-slate-200 dark:border-slate-700 pb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="license_number" value="{{ $broker->license_number }}" class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none dark:bg-slate-700 dark:text-slate-200">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Comisión (%)</label>
                                <input type="number" name="commission_rate" value="{{ $broker->commission_rate }}" step="0.01" min="0" max="100" class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none dark:bg-slate-700 dark:text-slate-200">
                            </div>
                        </div>
                        ...
                        <div class="mt-6">
                            ...
                            <select name="status" class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none dark:bg-slate-700 dark:text-slate-200">
                                ...
                                <option value="inactive" {{ $broker->status === 'inactive' ? 'selected' : '' }}>✕ Inactivo</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="bio" rows="4" class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none dark:bg-slate-700 dark:text-slate-200">{{ $broker->bio }}</textarea>
                    </div>

                    <div class="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-4 text-xs text-slate-600 dark:text-slate-400 space-y-1">
                        <p>✓ Creado: {{ $broker->created_at->format('d/m/Y H:i') }}</p>
                        <p>↻ Actualizado: {{ $broker->updated_at->format('d/m/Y H:i') }}</p>
                    </div>
