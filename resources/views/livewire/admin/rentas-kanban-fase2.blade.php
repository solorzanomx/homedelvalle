...
<div id="kanban2-toast" style="position:fixed;top:1.25rem;right:1.25rem;z-index:9999;display:none;max-width:320px;"
...
<div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:.75rem 1.1rem;margin-bottom:1rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap;">
    ...
    <select wire:model="bulkStage" style="border:1px solid #bfdbfe;border-radius:7px;padding:.3rem .65rem;font-size:.75rem;color:#0f172a;">
        ...
        @foreach(\App\Models\Operation::RENTA_STAGES as $s)
        @if(in_array($s, ['lead','contacto','visita','exclusiva','publicacion','busqueda','investigacion','contrato','entrega','cierre']))
...
{{-- ── Indicadores de etapa (leyenda visual) ───────────────────────────────── --}}
<div style="display:flex;gap:.5rem;margin-bottom:.85rem;flex-wrap:wrap;">
    @php
    $etapasColoc = [
        'captacion'   => ['lead','contacto','visita','exclusiva','publicacion'],
        'colocacion'  => ['busqueda','investigacion','contrato'],
        'cierre'      => ['entrega','cierre'],
    ];
    $grupoColors = [
        'captacion'  => ['bg' => '#eff6ff', 'border' => '#bfdbfe', 'text' => '#1D4ED8', 'label' => 'Preparación'],
        'colocacion' => ['bg' => '#fffbeb', 'border' => '#fde68a', 'text' => '#92400e', 'label' => 'Colocación'],
        'cierre'     => ['bg' => '#f0fdf4', 'border' => '#bbf7d0', 'text' => '#166534', 'label' => 'Cierre'],
    ];
    @endphp
    @foreach($grupoColors as $grupo => $colors)
    <span style="font-size:.62rem;font-weight:700;padding:.2rem .65rem;border-radius:9999px;background:{{ $colors['bg'] }};border:1px solid {{ $colors['border'] }};color:{{ $colors['text'] }};">
        {{ $colors['label'] }}
    </span>
    @endforeach
    <span style="font-size:.62rem;color:#94a3b8;padding:.2rem .65rem;">Arrastra las tarjetas para avanzar etapa</span>
</div>
...
<div style="display:flex;gap:.65rem;overflow-x:auto;padding-bottom:1.25rem;align-items:flex-start;">
    ...
    // Color de acento por grupo de etapa
    $stageAccent = [
        'lead'          => '#3B82C4',
        'contacto'      => '#3B82C4',
        'visita'        => '#3B82C4',
    ...
    @php
        ...
        $accent    = $stageAccent[$stage] ?? '#3B82C4';
        $borderColor = $hasOver ? '#ef4444' : $accent;
    @endphp
    <div style="min-width:210px;flex-shrink:0;">
        {{-- Header columna --}}
        <div style="display:flex;align-items:center;justify-content:space-between;padding:.45rem .7rem;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px 8px 0 0;border-bottom:2px solid {{ $borderColor }};">
            ...
            <span style="background:{{ $hasOver ? '#fef2f2' : '#f1f5f9' }};color:{{ $hasOver ? '#ef4444' : '#64748b' }};font-size:.62rem;font-weight:700;border-radius:9999px;padding:.1rem .45rem;">{{ $cardCount }}</span>
        </div>

        {{-- Cards —sortable-- --}}
        <div class="kanban2-col"
             data-stage="{{ $stage }}"
             wire:ignore
            ...
            <div class="kanban2-card"
                ...
                <div style="display:flex;align-items:flex-start;gap:.5rem;">
                    ...
                    <div style="flex:1;min-width:0;">
                        <a href="{{ route('admin.rentas.gestion.show', $card['id']) }}"
                           style="font-size:.75rem;font-weight:600;color:#0f172a;text-decoration:none;line-height:1.3;display:block;"
                           onclick="event.stopPropagation()">
                            {{ Str::limit($card['address'], 26) }}
                        </a>
                        <p style="font-size:.65rem;color:#64748b;margin-top:.05rem;">{{ $card['colony'] }}</p>
                    </div>
                </div>

                {{-- Datos --}}
                <div style="display:flex;flex-wrap:wrap;gap:.3rem .6rem;margin-top:.45rem;">
                    @if($card['rent'])
                    <span style="font-size:.65rem;font-weight:600;color:#1D4ED8;">${{ number_format($card['rent']) }}/mes</span>
                    @endif
                    @if($card['rooms'])
                    <span style="font-size:.65rem;color:#64748b;">{{ $card['rooms'] }} rec.</span>
                    @endif
                    @if($card['area'])
                ...
                {{-- Cliente (inquilino prospecto) --}}
                @if($card['client_name'] !== 'Sin cliente')
                <p style="font-size:.62rem;color:#94a3b8;margin-top:.35rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
...
<script>
document.addEventListener('livewire:initialized', () => { initKanban2(); });
document.addEventListener('livewire:updated',     () => { initKanban2(); });

function initKanban2() {
    document.querySelectorAll('.kanban2-col').forEach(col => {
        if (col._sortable) col._sortable.destroy();

        col._sortable = Sortable.create(col, {
            group:      'colocacion',
            animation:  150,
            ghostClass: 'kanban2-ghost',
            dragClass:  'kanban2-drag',
            handle:     '.kanban2-card',
            onEnd(evt) {
                ...
                Livewire.dispatch('card-moved-fase2', {
                    operationId: opId,
                    newStage:    newStage,
                    oldStage:    oldStage,
                });
...
<style>
.kanban2-ghost { opacity: .35; border: 2px dashed #10b981 !important; }
.kanban2-drag  { box-shadow: 0 8px 24px rgba(0,0,0,.15) !important; transform: rotate(1.5deg); }
</style>
